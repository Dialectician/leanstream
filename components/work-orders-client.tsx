...
export function WorkOrdersClient({
  ...
  const [currentItem, setCurrentItem] = useState<ItemWithAssemblies | null>(
    null
  );
  const [currentAssemblies, setCurrentAssemblies] = useState<number[]>([]);

  // State for new client creation
  const [clientTab, setClientTab] = useState<"existing" | "new">("new");
  ...
  const handleFormSubmit = () => {
    ...
    startTransition(async () => {
      const result = await (isEditing
        ...
        : createWorkOrderWithItems(formData));

      if (result.success) {
        if (!isEditing && result.data) {
          setOrders((currentOrders) => [
            result.data as OrderWithDetails,
            ...currentOrders,
          ]);
        }
        resetDialogs();
  ...
  const handleDelete = (orderId: number) => {
    if (
      window.confirm("Are you sure? This will permanently delete the order.")
    ) {
      startTransition(async () => {
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    {order.status !== "Completed" ? (
                      ...
                      <DropdownMenuItem
                        onClick={() => onStatusUpdate(order.id, "Planned")}
                      >
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <Dialog
        open={isModalOpen}
        onOpenChange={(isOpen) => !isOpen && resetDialogs()}
      >
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              {editingOrder
                ? `Edit Work Order: ${editingOrder.orderNumber}`
                : "Create New Work Order"}
            </DialogTitle>
            <DialogDescription>
              {editingOrder
                ? "Update the details for this work order."
                : "Follow the steps to build and create a new work order."}
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid col-span-2 gap-2">
                  <Tabs
                    value={clientTab}
                    onValueChange={(value) =>
                      setClientTab(value as "existing" | "new")
                    }
                    className="w-full"
                  >
                ...
                <div className="grid gap-2">
                  <Label>Fusion 360 Link</Label>
                  <Input
                    value={fusionLink}
                    onChange={(e) => setFusionLink(e.target.value)}
                    placeholder="https://fusion360.autodesk.com/..."
                  />
                </div>
                <div className="grid gap-2">
                  <Label>Katana Link</Label>
                  <Input
                    value={katanaLink}
                    onChange={(e) => setKatanaLink(e.target.value)}
                    placeholder="https://katanamrp.com/..."
                  />
                </div>
              </div>
              <div className="pt-4">
                <h3 className="font-semibold mb-2">
                  Items on Order ({stagedItems.length})
                </h3>
                <div className="border rounded-lg p-2 min-h-[80px] space-y-1">
                  {stagedItems.length > 0 ? (
                    stagedItems.map((item, index) => (
                      <div
                        key={index}
                        className="flex justify-between items-center text-sm p-1 hover:bg-muted/50 rounded-md"
                      >
                        <span>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-2">
                    <p className="text-sm font-medium">
                      Select required assemblies:
                    </p>
                    {currentItem.itemAssemblies.length > 0 ? (
                        ...
                        >
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => {
                              setCurrentAssemblies((prev) =>
                                checked
                                  ? [...prev, ia.assembly.id]
                                  : prev.filter((id) => id !== ia.assembly.id)
                              );
                            }}
                          />
                          <label
                            htmlFor={`assembly-${ia.assembly.id}`}
                            className="text-sm"
                          >
                            {ia.assembly.name}
                          </label>
                      ...
                      <p className="text-sm text-muted-foreground">
                        No assemblies configured for this item.
                      </p>
                    )}
                  </div>
                </div>
          ...
          <DialogFooter>
            {dialogStep === 1 && (
              <Button onClick={() => setDialogStep(2)} disabled={!orderNumber}>
                Add/Configure Items <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            )}
            {dialogStep === 2 && !currentItem && (
              <Button variant="secondary" onClick={() => setDialogStep(1)}>
                Back to Details
              </Button>
            )}
            {dialogStep === 2 && currentItem && (
              <Button variant="secondary" onClick={() => setCurrentItem(null)}>
                Change Item
              </Button>
            )}
            {dialogStep === 2 && currentItem && (
              <Button onClick={handleAddItemToStage}>Add Item to Order</Button>
            )}
            <Button onClick={resetDialogs} variant="outline">
              Cancel
            </Button>
            {dialogStep === 1 && stagedItems.length > 0 && (
